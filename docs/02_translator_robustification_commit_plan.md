...
- **T06e — flip default + cost note.** Dispatcher uses the structured pipeline by default; single-call path retained behind flag for batch. *Gate:* golden harness green end-to-end on the structured path; per-assignment `verbatim_span`s present in the result for audit.
...
| **D4** | **Claude structured-output mechanism.** T02 assumes `claude-agent-sdk` can hard-constrain output to a JSON schema via tool-use (OpenAI uses `response_format`). The current Claude path is text + regex; this capability is **asserted in the design doc but unverified** in this SDK version. | **T02** (Claude path) | Spike in T02: confirm a forced-schema tool call works with `max_turns=1`. If not, fall back to brace-matching + strict post-validation on the Claude path (OpenAI stays strict-bound). A2's *validation* guarantees hold either way; only the *enforcement* mechanism differs. |

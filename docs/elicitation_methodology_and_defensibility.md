...
**Applied to the default seeds (as shipped).** The relevance constraint is taken literally in the shipped Hormuz defaults. Because the targets are *probabilistic judgments about crisis dynamics* (incidents, escalation, market and supply consequences), the seeds ask about the same kind of quantity — incident counts, days of disruption, threat frequencies, premium and price spikes — and **not** static energy-system magnitudes (VLCC capacity, flow volumes), which would test numeracy the targets never exercise. An earlier draft used the magnitude seeds; they were rejected on exactly this relevance ground. Two residual mismatches remain and are not hidden. The *format* gap: the seeds are scored as continuous Cooke quantiles while the targets are categorical CPT columns — the proper-scoring/Brier route (§6.1) is what closes it. The *contamination* gap: these are retrodictive, lookup-able quantities, which only genuinely-prospective seeds remove. The source-attribution discard (§8.5b) plus these relevance-matched defaults are the interim mitigation; the Brier scoring path and a prospective-seed log are the real fixes, both still open.
...
### 8.5b What the prompts and seeds actually enforce (as shipped)

The mitigations above are not aspirational — they are compiled into the prompt builders and the default seed set the runner uses. Concretely:

- **Independence (anti-sycophancy).** Each agent is called in its own request and never sees another agent's answer; the persona text states it is "consulted independently." There is no discuss-and-revise round that could manufacture consensus. The pooled mean and the spread that sets κ are therefore functions of genuinely independent round-1 estimates.
- **Adversarial diversity.** The role-recruiting prompt must return distinct disciplinary lenses including at least one **red-team skeptic** and, for panels of ≥3, a **base-rate / outside-view forecaster**. Those two role families receive an extra persona directive (the skeptic argues for *wider* uncertainty; the base-rater anchors on reference classes), so the role does real cognitive work rather than being a label.
- **Anti-overconfidence.** Seed prompts use calibration-training language — the outer quantiles "should be wide enough that the truth falls inside about 90% of the time; most experts and models make these too narrow." CPT prompts require **reserving probability mass for surprise** (no 0/1 unless logically necessary). Both compose with the κ ladder, whose calibration gate caps an over-tight panel at `normal`.
- **Anti-anchoring / base-rate neglect.** Every reasoning directive forces the **outside view first** (reference-class base rate) and a **consider-the-opposite** step (one reason the estimate could be too high and one too low) before the model commits.
- **Anti-contamination (prompt + seed design).** Both seed and CPT prompts instruct the model to **reason from structure, not from a specific recalled figure/event**. The default seeds were redesigned to match the *target judgment* — probabilistic crisis dynamics — asking about incident counts, disruption days, threat frequencies, and premium/price spikes rather than static energy-system magnitudes (the relevance argument of §8.3). They remain retrodictive — a bootstrap, not a certificate.
- **Source-attribution discard (wired).** Each seed answer carries a self-reported `basis` ∈ {estimate, recall}; a seed a *majority* of the panel admits recalling is **discarded before scoring** (it measures memory, not calibration), and if every seed is discarded the panel falls back to equal weighting. The per-agent/per-seed outcomes are summarised onto the run — the `source_attribution_probe` of §8.3, now invoked at run time rather than only specified. This is self-report — gameable and imperfect — so it is a logged *signal and filter*, not a guarantee; the perturbation/canary and cross-model-variance probes of §8.3 remain stronger, not-yet-wired options.
- **Coherence.** CPT prompts require the table to be **monotone** across escalatory parent configurations, reducing the incoherent columns that arise when each is reasoned in isolation.
- **Honest weighting.** Weights come from role-neutral seed scoring; per §8.2 the role-conditioned CPT is a different estimator, so roles are treated as a considerations-diversification device behind the pooled mean and spread, **not** as separately-calibrated estimators. The run's audit note states this explicitly.

The residual, deliberately not papered over: a single-base-model panel is still correlated (κ is discounted for it but correlation is hard to kill without a second model), and seed calibration on retrodictive lookup-able quantities carries the transfer gap of §8.3. The defensible claim remains *measured and mitigated*, not *reliable*.

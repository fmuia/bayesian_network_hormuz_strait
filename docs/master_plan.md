...
The engineering plans run **sequentially** (2 → 3 → 4 → 5) with internal items in each plan having their own ordering. Sequencing is by dependency, not just convenience. **Plan 1 is the latent-regime reframing** — both the conceptual decision (already made) and the engineering implementation that delivers it. The conceptual half underlies the specification of Plans 2–5; the engineering half slots into Plan 3's timeline between Phase 1 (`PgmpyBackend` wrapped under `NetworkSpec`) and Phase 2 (`PymcBackend`), with Plan 1 explicitly owning the work. See §3 below for the full treatment.
...
The full work programme has five plans. **Plan 1** is the latent-regime reframing — the most consequential conceptual decision the project has made (already resolved), plus the engineering implementation that delivers it (not yet started). **Plans 2–5** are the four sequential engineering plans.
...
- **Section B — Engineering implementation (⬜ not started).** `LatentRegime` flag on `NetworkSpec`; anchor-derived emission CPTs and regime CPT $P(S \mid M, C)$ in `src/cpt_data.py` (replaced by Plan 4 Layer 2 elicitation when that lands); `PgmpyBackend` builds the latent regime using direct variable elimination for the posterior $P(S \mid E)$, with a three-clamped-inferences pattern for Bayes-factor extraction. PyMC-native support for the latent regime is added later, as part of Plan 3 Phase 2 when `PymcBackend` itself lands.
...
**Sequencing.** The engineering implementation is gated on **Plan 3 Phase 1** (`PgmpyBackend` wrapped under the new `NetworkSpec` interface) shipping. The math is implementable in pgmpy today, so Plan 1 ships in the backend the repo already uses; PyMC-native support is added later as part of Plan 3 Phase 2, alongside whatever else is needed to keep the rest of the system working under the new backend. Plan 3's later phases (Phase 3 continuous variables, Phase 4 Oil_Price migration) depend on Plan 1's pgmpy implementation having landed — continuous emissions are emissions *of the latent regime*. The execution order is therefore: Plan 3 Phases 0–1 → Plan 1 engineering (pgmpy) → Plan 3 Phase 2 (PymcBackend, including PyMC-native latent-regime support) → Plan 3 Phases 3–4. The framework write-up has no engineering prerequisites.
...
- **Input and reasoning**: B1a (article-level input + default per-source-type credibility), B1b (per-source credibility editing with history), B2 (span-grounded structured reasoning), B3 (abstention).
...
5 phases (0–4). **Plan 1's engineering implementation slots between Phase 1 and Phase 2** — Plan 1 builds the latent regime on top of the wrapped `PgmpyBackend` (the math is implementable in pgmpy today via direct VE), and Phase 2 then extends the latent regime to `PymcBackend` when PyMC arrives. The continuous-variable phases below build on top of Plan 1.
...
- *(Plan 1's engineering implementation runs here — latent regime built on top of `PgmpyBackend` via direct VE. Closes M1 and M7.)*
- **Phase 2**: build `PymcBackend` for discrete-only networks; extend latent-regime support to PyMC and modify whatever else is needed for the rest of the system to keep working under the new backend; validate parity against the pgmpy reference.
...
| `01_latent_regime_plan.md`      | **Plan 1.** Latent-regime reframing (Hormuz instance of the scenario-as-latent BN framework). Section A is the conceptual decision (resolved); Section B is the engineering implementation (not started, gated on Plan 3 Phase 1). Closes M1 and M7.                                                                                                                                                                                                                                |
...
| `02_translator_robustification.md` | **Plan 2.** Evidence-ingestion layer. 12 items, A1–E2.                                                                                                                                                                                                                                                                                                                                                                                 |
| `03_pymc_integration_plan.md`      | **Plan 3.** Inference engine. 5 phases, 0–4. Backend substrate for the latent-regime engineering work in Plan 1.                                                                                                                                                                                                                                                                                                                                              |
...
1. **Likelihood semantics for translator output (Plan 2 A1).** The LLM produces likelihood ratios `ε_s = P(article | state=s) / max_s'`, not posterior-shaped distributions. Closes M2 cleanly.
...
5. **Latent-regime topology (Plan 1).** $S \to D, T, P$ replaces $D, T, P \to S$; $S$ gains upstream parents $\text{Pa}(S) = \{M, C\}$ (the downstream-most mediator layer). Scenarios become latent regime variables generating intermediate outcomes; the current labelling CPT is removed; a regime CPT $P(S \mid M, C)$ is added. Bayes factors become first-class. This is the project's most consequential conceptual decision; the full treatment lives in `docs/01_latent_regime_plan.md` (Section A is the decision, including the underlying math; Section B is the engineering implementation; the framework that the plan instantiates is in `docs/scenario_bn_framework.md`). The implementation work is gated on Plan 3 Phase 1 shipping (so the latent regime lands in pgmpy first, in the backend the repo already uses); PyMC-native support is added later as part of Plan 3 Phase 2. The implementation has cross-cutting touches in Plans 2, 4, and 5 (summarised in Plan 1 Section C).
...
| 24  | Per-CPT $\kappa$ values for the latent-regime emission CPTs | Plan 1                               | Plan 1 ships with provisional uniform $\kappa = 10$ values on every emission CPT and on the regime CPT $P(S \mid M, C)$ (justified in Plan 1 Section B.1); per-CPT $\kappa$ becomes the elicited output of Plan 4 Layer 4. See Plan 1 Section B deliverables for the provisional-then-elicited pathway. Note: $\kappa$ is consumed by `PymcBackend` (hierarchical priors); pgmpy direct VE uses the point-estimate CPT, so the pgmpy-only Plan 1 MVP is insensitive to the $\kappa$ choice until Phase 2 lands. |
...
| 31  | Bayes-factor display in observed-node panel                 | Plan 5 C4                            | Depends on Plan 1's engineering landing. If still in pgmpy-only mode, fall back to delta display.                                                                                                                                            |

...
- **Section B — Engineering implementation (⬜ not started).** `LatentRegime` flag on `NetworkSpec`; anchor-derived emission CPTs and regime CPT $P(S \mid M, C)$ in `src/cpt_data.py` (replaced by Plan 4 Layer 2 elicitation when that lands); `PymcBackend` builds the latent regime natively; `PgmpyBackend` uses direct variable elimination for the posterior, with a three-clamped-inferences pattern for Bayes-factor extraction.
...
**Sequencing.** The engineering implementation is gated on **Plan 3 Phase 2** (PymcBackend for discrete networks) shipping. Plan 3's later phases (Phase 3 continuous variables, Phase 4 Oil_Price migration) in turn depend on Plan 1's implementation having landed — continuous emissions are emissions *of the latent regime*. The execution order is therefore: Plan 3 Phases 0–2 → Plan 1 engineering → Plan 3 Phases 3–4. The framework write-up has no engineering prerequisites.
...
This plan also builds the **shared substrate** (audit log, versioned-artefact pattern, HITL queue, source-credibility registry) that Plans 4 and 5 reuse.
...
5 phases (0–4). **Plan 1's engineering implementation slots between Phase 2 and Phase 3** — Plan 1 builds the latent regime on top of `PymcBackend`'s discrete substrate, and the continuous-variable phases below build on top of Plan 1.
...
- *(Plan 1's engineering implementation runs here — latent regime built on top of `PymcBackend`. Closes M1 and M7.)*
...
| C5  | Soft-evidence mismatch (code side of M2)    | Plan 2 + Plan 3 | A1 + Phase 4 |
...
| `01_latent_regime_plan.md`      | **Plan 1.** Latent-regime reframing (Hormuz instance of the scenario-as-latent BN framework). Section A is the conceptual decision (resolved); Section B is the engineering implementation (not started, gated on Plan 3 Phase 2). Closes M1 and M7.                                                                                                                                                                                                                                |
...
5. **Latent-regime topology (Plan 1).** $S \to D, T, P$ replaces $D, T, P \to S$; $S$ gains upstream parents $\text{Pa}(S) = \{M, C\}$ (the downstream-most mediator layer). Scenarios become latent regime variables generating intermediate outcomes; the current labelling CPT is removed; a regime CPT $P(S \mid M, C)$ is added. Bayes factors become first-class. This is the project's most consequential conceptual decision; the full treatment lives in `docs/01_latent_regime_plan.md` (Section A is the decision, including the underlying math; Section B is the engineering implementation; the framework that the plan instantiates is in `docs/scenario_bn_framework.md`). The implementation work is gated on Plan 3 Phase 2 shipping, slots between Plan 3's backend foundations and its continuous-variable phases, and has cross-cutting touches in Plans 2, 4, and 5 (summarised in Plan 1 Section C).
6. **Plan 1 ownership.** The conceptual decision, the engineering implementation, and the framework write-up are bundled into a single Plan 1 doc (`docs/01_latent_regime_plan.md`) that owns all three. M5 (material DAG omissions) remains a Gap pending a future structure-review workstream.

...
> **Status.** Living document. M5 (material DAG omissions) remains a Gap pending a future structure-review workstream. **Plan 3 (PyMC / continuous-variable backend) is 🅿️ SHELVED (executive decision, 2026-06-07)** to streamline the first implementation — the first pass stays on the existing pgmpy discrete path; see [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2. While parked, M2 (continuous-node facet), M3, and M4 are not closed.
>
...
> *Diagram is the **target** architecture. The Plan 3 inference-engine box (pgmpy + PyMC dual backend, `NetworkSpec`, continuous) is 🅿️ **shelved** for the first implementation — the live engine is the existing pgmpy discrete path (`src/inference.py`). See [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2.*
...
> **✅ POC shipped (2026-06-08).** After a skeptical gate the plan was re-scoped to a stakeholder POC: **T00–T06 + a slim in-session HITL (T12)** shipped on `explorations-dev-plan-2` — likelihood-ratio semantics (M2-interface/C5), schema hardening (C6/C7/C8), golden-set eval, article input + source credibility, relevance/abstention, span-grounded structured reasoning + injection defenses, and the Triage review queue. The **institutional layer (T07 ensemble, T08 prompt versioning, T09 sqlite provenance, T10 source history, T11 cross-model, T13 RAG, riders) is deferred post-POC** — parked in [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §3. Execution detail + status lives in [`02_translator_robustification_commit_plan.md`](02_translator_robustification_commit_plan.md). The summary below describes the full plan as designed.
...
> **🅿️ SHELVED (executive decision, 2026-06-07) — not on the current roadmap.** The whole of Plan 3 (dual backend, `NetworkSpec`, PyMC, hierarchical priors, continuous variables, the Oil_Price migration, operative per-CPT κ) is **parked** to streamline the first implementation. The first pass stays on the existing pgmpy discrete path. Rationale, impact, and the re-introduction trigger are in [`docs/06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2; the design is preserved verbatim in `docs/03_pymc_integration_plan.md`. **While parked, M2 (continuous-node facet), M3, and M4 are not closed** (see §4). The summary below describes the plan as designed, for when it is lifted back out.
...
> **Reconciled + split (2026-06-09).** After a skeptical gate, Plan 5 is split into a **POC slice** ([`05_dashboard_ui_plan.md`](05_dashboard_ui_plan.md) — commit-wise, A1/A2/A5 + C1/C2/C4/C7/C9/C11/C12/C15 + tests) and a **deferred remainder** ([`05_dashboard_ui_plan_deferred.md`](05_dashboard_ui_plan_deferred.md) — A3/A4/B1/B2/C3/C6/C8/C10/C13/C14/D1–D5, each with a trigger; indexed in [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §4). B3 is already satisfied and D3 near-done.
...
| M2  | Soft evidence semantics           | Plan 2 ✅ + ~~Plan 3~~ 🅿️                         | A1 ✅ shipped (T01, likelihood-ratio interface) + ~~Phase 3~~ 🅿️ continuous-node facet parked — see [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2 |
| M3  | Uniform κ                         | ~~Plan 3~~ 🅿️ + Plan 4                            | ~~Phase 3 (per-CPT κ)~~ 🅿️ + Layer 4 (elicitation provenance) — κ stays uniform/inert while Plan 3 is parked; see [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2 |
| M4  | Independent CPT column resampling | ~~Plan 3~~ 🅿️                                     | ~~Phase 3 (hierarchical priors)~~ 🅿️ — bolt-on Dirichlet resampling stays; parked, see [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2 |
...
| C5  | Soft-evidence mismatch (code side of M2)    | Plan 2 ✅ + ~~Plan 3~~ 🅿️ | A1 ✅ shipped (T01, discrete code side) + ~~Phase 3~~ 🅿️ — see [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2 |
| C6  | Translator validation permissive            | Plan 2 ✅       | A2 — shipped (T02) |
| C7  | Silent renormalization                      | Plan 2 ✅       | A2 — shipped (T02) |
| C8  | Regex JSON extraction fragile               | Plan 2 ✅       | A2 — shipped (T02, brace-matching parser) |
...
| `02_translator_robustification.md` | **Plan 2.** Evidence-ingestion layer (design rationale + math). 13 items, A1–E2 (incl. B4 untrusted-input handling), in 13 execution slots. Some sub-features parked in `06_dropped_to_simplify.md` §1.                                                                                                                                                                                                                                                                                                                                                                                 |
| `02_translator_robustification_commit_plan.md` | **Plan 2 — execution companion.** Commit-by-commit decomposition (T00–T13 + riders) with per-commit acceptance gates, a dependency graph, and an "Open decisions & risks" section. **POC shipped: T00–T06 + slim T12 ✅; T07/T08/T09/T10/T11/T13 + riders deferred (🅿️ §3 of `06`).** The *how/when* to Plan 2's *why*.                                                                                                                                                                                                                                              |
| `03_pymc_integration_plan.md`      | **Plan 3 — 🅿️ SHELVED (2026-06-07).** Inference engine (dual backend / PyMC / continuous variables). Parked to streamline the first implementation — see `06_dropped_to_simplify.md` §2. Design preserved for re-introduction. 5 phases, 0–4.                                                                                                                                                                                                                                                                                                                                              |
| `04_elicitation_tool_plan.md`      | **Plan 4.** Methodology layer. 6 active layers, 0–5 (Layer 6 deferred).                                                                                                                                                                                                                                                                                                                                                                |
| `05_dashboard_ui_plan.md`          | **Plan 5 — POC slice.** Dashboard UI and polish, reconciled + split 2026-06-09 into a commit-wise POC (foundation A1/A2/A5 + visuals C1/C2/C4/C7/C9/C11/C12/C15 + tests).                                                                                                                                                                                                                                                              |
| `05_dashboard_ui_plan_deferred.md` | **Plan 5 — deferred remainder.** The post-POC backlog (A3/A4/B1/B2/C3/C6/C8/C10/C13/C14/D1–D5), each with a deferral reason, re-introduction trigger, and acceptance gate. Indexed in `06_dropped_to_simplify.md` §4.                                                                                                                                                                                                                    |
| `06_dropped_to_simplify.md`        | **Parking lot.** Canonical record of capabilities deliberately deferred to streamline the first implementation, each with a substitute and a re-introduction trigger. Currently: §1 embedding-based translator features (Plan 2), §2 the PyMC / continuous-variable backend (Plan 3, shelved). |
...
1. **Dual-backend with capability-based dispatch. (🅿️ SHELVED 2026-06-07 — see [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2.)** A declarative `NetworkSpec` describes the model; a dispatcher routes to `PgmpyBackend` (fast exact for discrete) or `PymcBackend` (hierarchical priors, continuous variables, PyMC-native latent regime). Dashboard is backend-agnostic via a uniform `Posterior` interface. *Parked: the first implementation stays on the existing pgmpy discrete path with no `NetworkSpec` or backend abstraction.*
2. **PyMC is mandatory when continuous variables are present. (🅿️ SHELVED 2026-06-07 — moot while no continuous variables exist; see [`06_dropped_to_simplify.md`](06_dropped_to_simplify.md) §2.)** All-discrete networks let the user choose; mixed networks must use PyMC.
